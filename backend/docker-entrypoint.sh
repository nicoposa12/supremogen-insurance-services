...
nohup su -s /bin/sh -c "php artisan queue:work --sleep=10 --tries=3 --timeout=60 >> /var/www/html/storage/logs/queue.log 2>&1" www-data &

# Start scheduler loop in background (runs every 60 seconds) as www-data
echo "Starting scheduler loop..."
nohup su -s /bin/sh -c "while true; do php artisan schedule:run --no-interaction >> /var/www/html/storage/logs/scheduler.log 2>&1; sleep 60; done" www-data &
